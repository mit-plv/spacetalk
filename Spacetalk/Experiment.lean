...
namespace Df
  ...
  theorem DFG.PredicatedMultiStep.predicate_transfer (step : DFG.PredicatedMultiStep dfg P1 s1 s2)
    (h_P_node : ∀ node, P1.node node → P2.node node) (h_P_state : ∀ state, P1.state state → P2.state state)
    : dfg.PredicatedMultiStep P2 s1 s2 := by
    induction step with
    | refl h =>
      exact DFG.PredicatedMultiStep.refl (h_P_state _ h)
    | head node h_mem hd h_node h_s1 h_s2 tl ih =>
      exact DFG.PredicatedMultiStep.head node h_mem hd
        (h_P_node _ h_node) (h_P_state _ h_s1) (h_P_state _ h_s2) (ih h_P_node h_P_state)
  ...
  theorem DFG.PredicatedMultiStep.tail {dfg : DFG} {node : Node} (hd : dfg.PredicatedMultiStep P s1 s2) (h_mem : node ∈ dfg) (h_node : P.node node)
    (h_s : P.state s3) (step : node.Step s2 s3) : dfg.PredicatedMultiStep P s1 s3 := by
    induction hd with
    | refl h_s1 =>
      exact DFG.PredicatedMultiStep.single h_mem h_node h_s1 h_s step
    | head node' h_mem' hd h_node' h_s1 h_s2 tl ih =>
      exact DFG.PredicatedMultiStep.head node' h_mem' hd h_node' h_s1 h_s2 (ih h_node h_s step)
...
namespace Compiler
  ...
  macro "new_op_node_in_MergeTwo " maxId:term ", " e1:term "," e2:term ", " nid:term ", " op:term ", " ts:term : tactic =>
   `(tactic|
  ...
  lemma new_op_node_in_MergeTwo_left
    ...
    new_op_node_in_MergeTwo maxId, e1, e2, nid, op, ts

  lemma new_op_node_in_MergeTwo_right
    (h_mem : ⟨nid, .binOp op ts⟩ ∈ (compileAux (compileAux maxId e1).2 e2).1.dfg)
    : ⟨nid, .binOp op ((ts.map
                        (λ t => if t = (compileAux maxId e1).1.ret then ⟨(compileAux (compileAux maxId e1).2 e2).2, 0⟩ else t)).map
                        (λ t => if t = (compileAux (compileAux maxId e1).2 e2).1.ret then ⟨(compileAux (compileAux maxId e1).2 e2).2, 1⟩ else t))⟩
        ∈ (MergeTwo e1 e2 maxId).1 := by
  ...
  macro "port_ne_ret_in_op_step_merge_left " maxId:term ", " e1:term ", " e2:term ", " op:term ", " ts:term ", " nid:term ", " h_mem:term : tactic =>
    `(tactic|
     (rw [not_or]
      apply And.intro
      · intro heq
  ...
  macro "state_pred_in_op_step_merge_left " maxId:term ", " e1:term ", " e2:term ", " s:term ", " hs:term : tactic =>
    `(tactic|
      (intro port h
        ...
        · cases hd with
          | binOp h1 h2 =>
            ...
            have h_fst_ne_ret : ¬(⟨nid, 0⟩ = (compileAux maxId e1).1.ret ∨ ⟨nid, 0⟩ = (compileAux (compileAux maxId e1).2 e2).1.ret) := by
              port_ne_ret_in_op_step_merge_left maxId, e1, e2, op, ts, nid, h_mem
            ...
            apply Nat.ne_of_lt
            trans
            · exact compile_binop_consumer_lt_max h_mem p h_p_mem
            · exact compile_maxId_lt
        · simp
        · state_pred_in_op_step_merge_left maxId, e1, e2, s1, h_s1
        · state_pred_in_op_step_merge_left maxId, e1, e2, s2, h_s2

  macro "port_ne_ret_in_op_step_merge_right " h_mem:term : tactic =>
    `(tactic|
     (rw [not_or]
      apply And.intro
      · apply Port.node_ne
        ...
        apply Nat.ne_of_lt
        apply Nat.lt_of_lt_of_le
        · exact compileAux_ret_lt_newMax
        · exact compileAux_maxId_le_id _ $h_mem
      · intro h
        have := (compileAux_ret_iff_output _ $h_mem).mp (Port.mk.inj h).left
        contradiction))

  macro "state_pred_in_op_step_merge_right " maxId:term ", " e1:term ", " s:term ", " hs:term : tactic =>
    `(tactic|
      (intro port h
       have : $s (compileAux $maxId $e1).1.ret = [] :=
         $hs _ (Or.intro_left _ (compileAux_ret_lt_newMax))
       aesop))
  ...
  theorem op_multi_step_to_merged_right {e1 e2 : Exp} {maxId : Nat} {s1 s2 : State}
   ...
   : (MergeTwo e1 e2 maxId).1.PredicatedMultiStep
       ...
       (MergedState e1 e2 maxId s1) (MergedState e1 e2 maxId s2) := by
    generalize h_dfg : (compileAux maxId e1).1.dfg = dfg at *
    generalize h_Pred : (⟨NodePredicate.IsOp, NidRange (compileAux maxId e1).2 (compileAux (compileAux maxId e1).2 e2).2⟩ : Predicate) = Pred at *
    induction step with
    | refl h_s =>
      rename_i s
      simp_rw [←h_Pred] at *
      ...
      intro port h
      have := h_s port (by omega)
      have : s (compileAux maxId e1).1.ret = [] := by
        apply h_s
        apply Or.intro_left
        exact compileAux_ret_lt_newMax
      aesop
    | head node h_mem hd h_node h_s1 h_s2 tl ih =>
      ...
      | ⟨nid, .binOp op ts⟩, _ =>
        ...
        let newTs :=
          (ts.map (λ t => if t = (compileAux maxId e1).1.ret then ⟨newRet, 0⟩ else t)).map
            (λ t => if t = (compileAux (compileAux maxId e1).2 e2).1.ret then ⟨newRet, 1⟩ else t)
        apply DFG.PredicatedMultiStep.head ⟨nid, .binOp op newTs⟩ (new_op_node_in_MergeTwo_right h_mem) _ _ _ _ (ih trivial)
        · cases hd with
          | binOp h1 h2 =>
            have h_nid_ne_ret : nid ≠ (compileAux (compileAux maxId e1).2 e2).2 :=
              Nat.ne_of_lt (compile_id_lt_max _ h_mem)
            have h_fst_ne_ret : ¬(⟨nid, 0⟩ = (compileAux maxId e1).1.ret ∨ ⟨nid, 0⟩ = (compileAux (compileAux maxId e1).2 e2).1.ret) := by
              port_ne_ret_in_op_step_merge_right h_mem
            have h_snd_ne_ret : ¬(⟨nid, 1⟩ = (compileAux maxId e1).1.ret ∨ ⟨nid, 1⟩ = (compileAux (compileAux maxId e1).2 e2).1.ret) := by
              port_ne_ret_in_op_step_merge_right h_mem
            have h_fst_eq : MergedState e1 e2 maxId s1 ⟨nid, 0⟩ = s1 ⟨nid, 0⟩ := mergedState_eq h_nid_ne_ret h_fst_ne_ret
            have h_snd_eq : MergedState e1 e2 maxId s1 ⟨nid, 1⟩ = s1 ⟨nid, 1⟩ := mergedState_eq h_nid_ne_ret h_snd_ne_ret
            ...
            exact compile_binop_consumer_lt_max h_mem p h_p_mem
        · simp
        · state_pred_in_op_step_merge_right maxId, e1, s1, h_s1
        · state_pred_in_op_step_merge_right maxId, e1, s2, h_s2
    ...
    | plus e1 e2 =>
      ...
      | plus eval1 eval2 =>
        ...
        · have ih1 := op_multi_step_to_merged_left (e2 := e2) e1_ops
          ...
          have hd := DFG.PredicatedMultiStep.merge_disjoint_nid_ranges_plus_port ih1 ih2
                    compile_maxId_lt compile_maxId_lt (le_refl _) (by simp) (le_refl _)
          have hd := hd.predicate_transfer (P2 := ⟨NodePredicate.IsOp, NidRange maxId (compileAux maxId (e1.plus e2)).2⟩)
            (λ _ h => h)
            (λ s h => by
              intro port h'
              apply h
              simp only [compileAux] at h'
              omega)
          apply DFG.PredicatedMultiStep.tail
            (node := ⟨(compileAux (compileAux maxId e1).2 e2).2, .binOp .plus [⟨(compileAux (compileAux maxId e1).2 e2).2 + 1, 0⟩]⟩)
            hd
          · simp
          · simp
          · simp_all only [MarkedDFG.initialState, VarMap.initialState, mergeTwo, List.map_map, compileAux, NidRange,
              ge_iff_le, MarkedDFG.finalState, ite_eq_right_iff, List.cons_ne_self, imp_false]
            intro port a
            apply Aesop.BuiltinRules.not_intro
            intro a_1
            subst a_1
            simp_all only [Nat.add_le_add_iff_left, Nat.reduceLeDiff, or_false]
            suffices maxId < (compileAux (compileAux maxId e1).2 e2).2 + 1 by omega
            trans (compileAux (compileAux maxId e1).2 e2).2
            · trans (compileAux maxId e1).2
              · exact compile_maxId_lt
              · exact compile_maxId_lt
            · simp
          · apply Node.Step.subst_right (.binOp (by simp) (by simp))
            have : ⟨(compileAux (compileAux maxId e1).2 e2).2 + 1, 0⟩ ≠ (compileAux maxId e1).1.ret := by
              apply Port.node_ne
              symm
              apply Nat.ne_of_lt
              trans (compileAux (compileAux maxId e1).2 e2).2
              · trans (compileAux maxId e1).2
                ...
                · exact compile_maxId_lt
              · simp
            have : ⟨(compileAux (compileAux maxId e1).2 e2).2 + 1, 0⟩ ≠ (compileAux (compileAux maxId e1).2 e2).1.ret := by
              apply Port.node_ne
              symm
              apply Nat.ne_of_lt
              trans (compileAux (compileAux maxId e1).2 e2).2
              · exact compileAux_ret_lt_newMax
              · simp
            have : (compileAux (compileAux maxId e1).2 e2).1.ret ≠ (compileAux maxId e1).1.ret := by
              apply Port.node_ne
              symm
              apply Nat.ne_of_lt
              trans (compileAux maxId e1).2
              · exact compileAux_ret_lt_newMax
              · exact compileAux_max_lt_ret
            -- proof below generated by `aesop`
            simp_all only [MarkedDFG.initialState, VarMap.initialState, mergeTwo, List.map_map, compileAux, ne_eq,
              State.pushAll, BinOp.denote, State.union, mergedState, ↓reduceIte, MarkedDFG.finalState,
              List.singleton_append, List.head_cons, Port.mk.injEq, one_ne_zero, and_false, List.nil_append,
              List.foldl_cons, List.foldl_nil]
            ext x_1 i a : 3
            simp_all only [State.push, State.pop, Port.mk.injEq, add_right_eq_self, one_ne_zero, zero_ne_one,
              and_self, ↓reduceIte, and_true, State.union, mergedState, or_self, MarkedDFG.finalState,
              List.append_nil, List.concat_eq_append, List.nil_append, and_false, List.tail_cons,
              List.singleton_append, Option.mem_def]
            apply Iff.intro
            · intro a_1
              split
              next h =>
                  ...
                  next h_2 =>
                    ...
                    next h_3 =>
                      ...
                      next h_4 =>
                        ...
                        next h_5 =>
                          subst h_4
                          simp_all only [List.singleton_append, or_false, not_true_eq_false]
                      next h_4 =>
                        split at a_1
                        next h_5 =>
                          subst h_5
